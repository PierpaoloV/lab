function [x] = evaluation3(trans, metric, layers, images)
%This is the optimization/evaluation
% In this version, the images are inverted if the mode is higher than the
% threshold otsu, reverting the inversion back in the end if it was the
% fixed one.
...
%                     fprintf('The Fixed image looks to be inverted, so we apply an inversion before registration');
                ...
                [x]=fminsearch(@(x)affine_registration_function(x,scale,Im,If,metric, trans),...
                    ...
                    'TolFun', 1.000000e-4,'TolX',1.000000e-4, 'MaxFunEvals', 1000*length(x)));
                if (ss == 1)
                [x]=fminsearch(@(x)affine_registration_function(x,scale,Im,If,metric,trans),...
